...
# When no certificate is provided, keep HTTP and forward directly to front TG
...
# When only HTTP is present, also split paths for back and grafana
...
resource "aws_lb_listener_rule" "grafana_rule_http" {
  count        = length(aws_lb_listener.http_forward)
  listener_arn = aws_lb_listener.http_forward[0].arn
  priority     = 20
  action {
