...
module "asg_back" {
  ...
  user_data            = templatefile(
    ...
    {
      MONGO_URI = var.enable_compute ? "mongodb://${aws_instance.db[0].private_ip}:27017/rdv" : "mongodb://127.0.0.1:27017/rdv"
    }
  )
...
resource "aws_codedeploy_deployment_group" "backend_dg" {
  ...
  autoscaling_groups    = [module.asg_back[0].this_asg_name]
  deployment_style {
